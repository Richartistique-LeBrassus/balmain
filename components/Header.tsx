...
import { ClerkLoaded, SignedIn, SignInButton, UserButton, useUser, useSession } from '@clerk/nextjs';
import Form from 'next/form';
import Link from 'next/link';
import { PackageIcon, TrolleyIcon, BasketIcon } from '@sanity/icons';
...
const Header = () => {
  ...
  const createClerkPasskey = async () => {
    try {
      ...
      const error = err?.errors?.[0]?.code || err?.code;
  
      if (error === "passkey_registration_cancelled") {
          ...
          <ClerkLoaded>
            ...
            {user?.passkeys.length === 0 && (
              <button 
                ...
                animate-pulse text-blue-700 font-bold py-2 px-4 rounded
